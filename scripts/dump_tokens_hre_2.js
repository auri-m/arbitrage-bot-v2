...
const main = async () => {
    ...
    const price_before_swap_on_dex_to_swap =
        await calculatePrice(dex_to_swap_pair_contract)
    const price_on_other_dex =
        await calculatePrice(other_dex_pair_contract)
    console.log("swaping...");
    ...
    const price_after_swap_on_dex_to_swap =
        await calculatePrice(dex_to_swap_pair_contract)
    ...
    console.table({
        'Price Before': `1 ${token_to_receive.symbol} = ${Number(price_before_swap_on_dex_to_swap).toFixed()} ${token_to_swap.symbol}`,
        'Price After': `1 ${token_to_receive.symbol} = ${Number(price_after_swap_on_dex_to_swap).toFixed()} ${token_to_swap.symbol}`,
        'Price On Other Dex': `1 ${token_to_receive.symbol} = ${Number(price_on_other_dex).toFixed()} ${token_to_swap.symbol}`,
    })
    ...
    console.log("\nNew price calculation by indexes")
    console.table({
